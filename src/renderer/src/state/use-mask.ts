...
import { useStore, activeLayerOf } from './store'
import { computeMask } from '../ui/mask'

/** Recomputes the drawable-area mask whenever the ACTIVE LAYER's underlay image,
 *  mask settings, or canvas size change (each song's chart brings its own drawable
 *  area), and stores the result (runtime only) for the editor. */
export function useMask(): void {
  const dataUrl = useStore((s) => activeLayerOf(s.chart)?.underlay?.dataUrl)
  const enabled = useStore((s) => activeLayerOf(s.chart)?.underlay?.mask?.enabled ?? false)
  const invert = useStore((s) => activeLayerOf(s.chart)?.underlay?.mask?.invert ?? false)
