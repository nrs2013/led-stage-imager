...
```
- **Windows配布**：`npm run build:win` で完結（2026-07-07修正済＝NDI DLL存在チェック→typecheck→コンパイル→**x64**指定でビルド。→ `dist/decor-studio-1.0.0-setup.exe`）。WindowsのMIDIはWeb MIDI経由（LEARNボタンで初期化・CoreMIDIはMac専用）。Mac版はAppleシリコン向け。DMG/exeとも**無署名**＝受け取った人への開き方説明が必須（デスクトップ「配布用」フォルダのREADME参照）。
...
- **commit / push は完成単位で自動**（2026-07-03 のむさん明言「もう自動でお任せ」）。typecheck＋テスト＋ビルドが通ってひと仕事が完成したら、聞かずにコミット→プッシュまで済ませて報告に含める。途中の壊れた状態は上げない。push 前に `git fetch` でリモート先行を確認。force push 等の破壊系だけは必ず事前確認。
...
- **電飾(チャート)の Syphon/NDI 出力は常時**（2026-07-07 のむさん決定で Live/Edit 切替を廃止・`useChartOutput` が編集中も30fpsで送出・画像照明モード中は自動で黙る）。LiveView は表示専用（?live 起動の出力専用機と Fullscreen 窓用）。「Resolume に出ない」の一次疑いは Resolume 側のソース選択。
