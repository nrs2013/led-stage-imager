...
export function ManualFaders({ onClose }: { onClose: () => void }): React.JSX.Element {
  ...
  const sequence = useMemo((): Fixture[] => {
    ...
    fixtures = fixtures.filter((f) => f.universe * 512 + f.start - 1 >= start)
    if (order === 'address') return fixtures.sort((a, b) => a.universe - b.universe || a.start - b.start)
    if (order === 'tap') {
      const rank = new Map(tapOrder.map((id, index) => [id, index]))
      return fixtures.sort((a, b) => (rank.get(a.id) ?? Number.MAX_SAFE_INTEGER) - (rank.get(b.id) ?? Number.MAX_SAFE_INTEGER))
    }
    return fixtures
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chart.fixtures, selectedFixtureIds.join('|'), startUniverse, startChannel, order, tapOrder])
  const lightOnly = (index: number): void => {
    if (!sequence.length) return
    const next = ((index % sequence.length) + sequence.length) % sequence.length
    setManualMode(true)
    setManualAll([0, 0, 0])
    setManualMany([sequence[next].id], litColor())
    setStep(next)
  }
  ...
  return <aside style={drawer}>
    ...
    <div style={box}>
      ...
      <div style={{ fontFamily: F.mono, fontSize: 11, color: C.hint, marginTop: 7, minHeight: 16 }}>{step >= 0 && sequence[step] ? `${step + 1}/${sequence.length}  ${formatDmx(sequence[step].universe, sequence[step].start)}` : `対象 ${sequence.length}`}</div>
    </div>
    <div style={box}>
