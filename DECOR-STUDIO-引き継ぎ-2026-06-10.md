...
- **くり抜きの可視化（6/10 のむさん要望）**：黒い絵×黒背景でくり抜きの縁が見えない問題 → **描けるエリア（透明くり抜き）の下に無彩色グレー(70,70,76)を敷く**（`MaskData.holes`・絵より先に描くので**絵そのものには一切色を載せない**）。**同日、旧シェード（描けない側を暗くするoverlay）は二重装備として撤去**（のむさん判断・グレーが境界係の本職に。絵は素の明るさで表示される）。
...
- **PIXEL島/DRAW島＋なぞり×自動清書（6/10 のむさんと設計相談→体感コア実装済み）**：
  ...
  - 残り（GO待ち）：⌘C/⌘Vスタンプ連打、Inspector数値仕上げ（長さ/角度）、DRAW島Penの二重ドット掃除、清書の効き具合調整。
- **「勝手に繋がる」廃止＋複数選択＋右クリック結合（6/10 のむさん「触りづらい」→同日修正・実技試験済み）**：
  - **Shift+クリックの自動継ぎ足しを廃止**（別の場所に棒を打ちたい時まで前の線に合体する事故のため）。繋ぐのは明示操作に一本化。
  - **バグ修理**：消しゴムで分割した断片に古い角情報（verts）が残り**編集不能**になっていた → `erase.ts` で**vertsを断片ごとに正しく引き継ぎ**（生き残った角は保持・範囲外は除去）。切った後も端を引っ張って伸ばせる。
  - **複数選択**：`store.selectedIds`。Selectツールで**空白からドラッグ＝囲み選択（ラバーバンド）**・**Shift+クリック＝追加/除外**・選択群をドラッグ＝**グループ移動**・Delete＝まとめて削除（1手）。白ハイライトは全選択分、ハンドルは単独選択時のみ。
  - **右クリック**：クリック＝**コンテキストメニュー**（「1本に結合（同じフェーダーで光る）」「削除」）／4px以上ドラッグ＝従来の掴んで移動（自動判別）。
  - **1本に結合**（`model/merge-runs.ts`）：選択した塗り線を**端が近い順に貪欲連結**（必要なら向き反転）し、**隙間は直線ドットで橋渡し**して1本のチェーン＝1 Fixtureに。消しゴムで切った2本の「やっぱりやめた」復縁にも使える。⌘Z 1手で戻る。
  - hitTestの感触メモ：クリック判定は**画面上6px**相当（ズームに応じて拡大）— 遠いズームでは線の近くの「空白」が掴み判定になる。囲み選択は線から少し離れた所から始めること。
- **⌘C/⌘V スタンプ連打（6/10 のむさん設計→同日実装・実技試験済み）**：
  - **⌘C**＝選択中の線たち（複数可）を控えにコピー → **⌘V**＝スタンプモード（Selectに自動切替・半透明お化けがカーソル追従・カーソル=copy）→ **クリックした座標を中心にペタッ**（セル整数オフセットで.5中心維持=クリスプ）→ **そのまま連続クリックで連打**、Escで終了。下部に案内トースト。
  - 番地は**コピー元と同じでクローン**（=同じフェーダーで一緒に光る・後からInspectorで変更可）。verts（角情報）も複製。1スタンプ=1 undo手。
  - **⌘C/⌘VもMacアプリのメニュー横取り対策済み**（menu Copy/Paste→`edit:copy/paste` IPC→renderer。テキスト入力中はmainの`webContents.copy()/paste()`でネイティブ動作維持）。ブラウザ(Web版)はkeydown経路。store: `clipboard/pasteArmed/copySelection/pasteAt`（store.test.ts 3本）。
- **矢印＝基本の家 化（6/10 のむさん要望・実技試験済み）**：
  - **空クリック＝ペースト位置マーク**（緑の十字を表示・store `pasteMark`）→ **⌘V＝マークのど真ん中にペースト**してマーク解除 → クリック→⌘V→クリック→⌘V…のリズムで連続配置。マーク無しの⌘Vは従来のゴースト追従スタンプモード。線クリック/囲み選択/Esc/⌘ペイント開始でマーク解除。
  - **矢印モードのまま⌘ドラッグ＝ドット描き**（`paintFromSelect`・Paintと同一機能: 軌跡塗り→自動清書・Shift直線・マスク尊重）。道具は矢印のまま戻る。Paint島/Eraserはそのまま存続。
