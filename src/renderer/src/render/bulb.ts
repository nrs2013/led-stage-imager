 ...
 *  colour — hue and gauge are derived here. Boosted top end: past ~92% the core
 *  blows out white and the bloom surges (のむさん「最後はもっと光った感じ」). */
export function drawBulbLit(
  ...
  if (style === 'frost') {
    ...
    if (r >= 14) {
      ...
      ctx.strokeStyle = rgba(hue, 0.85 * I)
      ...
      g1.addColorStop(0.4, rgba(hot, 0.9 * I))
