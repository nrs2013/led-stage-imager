...
- **残課題**（実機で詰める）：Arena連携の縦横比合わせ／白茶け修正／LEDドット描画（光る3mm）。**背景チャート実寸合わせは実装済み（2026-06-14・§7-4／§12・要実機目視）。**
- **⚠ 2026-06-14 追記（パッケージ版の致命バグ→修正済み・§13）**：`.app` を実機で開いたら**全フォント文字化け＝CSP に `font-src` が無く `data:` 埋込フォントが全ブロック**だった。`index.html` の CSP に `font-src 'self' data:`（＋動画用 `media-src 'self' blob:`・`img-src` に `blob:`）を足して解消。**これまで packaged UI を目視してなかったので発覚が遅れた**＝今後ビルドしたら必ず `.app` を開いて目視。
- **2026-06-14 追記**：ツールバーのロゴ（`editor/Toolbar.tsx`）も「DECOR STUDIO」→「**LED STAGE IMAGER**」に（改名漏れ解消）。**§7-4 は実機の Setup で動作確認済**（横12m→「1px=6.3mm・1m≈160px・縦≈6.8m」）。
...
- `src/renderer/src/editor/Toolbar.tsx`: 編集画面左上のロゴ（**2026-06-14 追加**：「DECOR STUDIO」→「LED STAGE IMAGER」白＋IMAGERシアン。当初の改名漏れを補修）
...
4. ~~**背景チャート実寸合わせ**~~ → **実装済み（2026-06-14・§12・要実機目視）**。Setup の「ステージ実寸 横(m)」で校正（`settings.stageWidthMm`）→部品ドロップ時に実寸mm→canvas px 変換（`model/scale.ts`）。canvas の px 数（=出力解像度）は据え置き（mm 化＝数千万画素は OOM のため）。
...
**最初にやること**：実機（`LED STAGE IMAGER.app`）＋ Resolume Arena で、**§6 Arena連携の縦横比**と **§7 の白茶け・LEDドット描画**を実機で詰める（**§7-4 背景チャート実寸は実装済み＝§12、まず実機目視で妥当性を確認**）。**メモリは §1 を厳守**（preview禁止・Workflow並列禁止・逐次・Codex切る）。

---

## 12. §7-4 背景チャート実寸合わせ（実装 2026-06-14・要実機目視）

**狙い**：チャート（背景PNG）ごとに縮尺がバラバラ → 「このチャートは横◯m」で校正し、§5 で実寸mm化した部品をその背景に対して**実物大**で置けるように。

**設計（メモリ安全＝重要）**：このアプリは **canvas のpx寸法＝出力解像度**。mm化（例 12m=12000px）すると数千万画素で **OOM 直行**（§1 最優先制約に違反）。なので **canvas は据え置き、チャートに実寸スケールを別途持たせ、部品ドロップの瞬間に mm→px 変換**する方式を採用。
- `Chart.settings.stageWidthMm?`（実寸の横幅mm）。在れば校正済み＝**1px = stageWidthMm / canvas.w (mm)**。無ければ従来通り（mm をそのまま px）。後方互換（既存・旧保存ファイルは未校正＝不変）。
- `src/renderer/src/model/scale.ts`（純粋関数・テスト9本＝`scale.test.ts`）：`mmPerPx(chart)` / `mmToCanvasPx(chart,mm)` / `stageWidthMeters(chart)`。
...
## 13. ⚠ CSP フォント文字化け修正（2026-06-14・パッケージ版の致命バグ）

**症状**：焼いた `.app` を開くと**ウィンドウは出るが全テキストが文字化け**（黒地に壊れたグリフ）＝「壊れて全文が開かない」。クラッシュではない（プロセスは生存）。

**原因**：`src/renderer/index.html` の CSP が `default-src 'self'; … img-src 'self' data:` で、**`font-src` が無かった**。Vite は小さいフォントサブセットを `data:font/woff2;base64,…` でHTMLに埋め込むが、`font-src` 未指定だと `default-src 'self'` にフォールバックして **`data:` フォントを全部ブロック** → フォント未適用で化ける。`dev:web`(ブラウザ)では出ず、**packaged の file:// + CSP でだけ出る**ので今まで気づかなかった（packaged UI を目視してなかった）。

**修正**：`index.html` の CSP を
`default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; media-src 'self' blob: data:`
に。`font-src ... data:`＝フォント解禁（フォントは受動的＝安全）、`media-src ... blob:`＝imagelightの動画(ObjectURL)も同じ穴に落ちるので予防、`img-src ... blob:`＝予防。

**確かめ方**：`ELECTRON_ENABLE_LOGGING=1 "…/Contents/MacOS/LED STAGE IMAGER" >/tmp/lsi.log 2>&1 &` で起動しログを `grep "violates the following Content Security Policy"` → **0件**ならOK。修正後、実機で StartScreen/編集画面/Setup すべて正常表示を目視確認済み。

**教訓（重要）**：**ビルドしたら必ず `.app` を `open` して目視**する（typecheck/test では文字化けは捕まらない＝描画・CSPはランタイム）。preview禁止(§1)でもこの目視だけは省かない。
